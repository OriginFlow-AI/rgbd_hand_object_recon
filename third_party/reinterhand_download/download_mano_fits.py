...
def download(capture_id):
    output_dir = ROOT / capture_id / 'mano_fits'
    for name in name_list[capture_id]:
        download_file(f"{BASE_URL}/{capture_id}/mano_fits/mano_fits.tar.gz{name}", output_dir)
    extract_multipart(output_dir, 'mano_fits.tar.gz')
