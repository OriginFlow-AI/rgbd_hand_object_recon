...
def run_reinterhand_best_right_icp(
    *,
    ...
    mesh_dir = Path(data_root) / capture / "mano_fits" / "meshes"
    ...
    target_cloud = scale_cloud(load_point_cloud(target_path), input_scale)
    ...
    for index, source_path in enumerate(source_paths, start=1):
        source_cloud = scale_cloud(load_point_cloud(source_path), input_scale)
    ...
    return ReinterHandIcpResult(status="ok", summary_path=summary_path, output_dir=output_dir, summary=summary)
